...
| strategy | object | `{"type":"Recreate"}` | Deployment strategy Use "Recreate" when persistence is enabled with RWO volumes to avoid file lock conflicts during rollouts (H2 database only supports single writer) |
